...
const AIAnalyzer = () => {
  ...
  const handleAutoCreateProject = async () => {
    if (!result?.extractedSkills?.length || !customProject) {
      setSnackbar({
        open: true,
        message: 'Proje oluşturmak için önce analiz yapmalısınız.',
        severity: 'warning',
      });
      return;
    }
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      if (inputType === 'existing') {
        ...
        extractedSkills = allSkills.filter(
          (item, idx, arr) => arr.findIndex((x) => normalize(x) === normalize(item)) === idx
        );
      } else {
      ...
      if (err?.response?.data?.error?.status === 'UNAVAILABLE') {
        setSnackbar({
          open: true,
          message: 'Yapay zeka servisinde yoğunluk var. Lütfen birkaç dakika sonra tekrar deneyin.',
          severity: 'warning',
        });
      }
      extractedSkills = [];
      ...
      const candidates = users
        ...
        .map((u) => {
          ...
          const missingSkills = extractedSkills.filter(
            (skill) => !u.skills.map((s) => normalize(s)).includes(normalize(skill))
          );
  ...
  const handleInvite = async (userId) => {
    ...
    try {
      ...
      if (errorMsg && errorMsg.includes('zaten bekleyen bir davet')) {
        setSnackbar({
          open: true,
          message: 'Bu kullanıcıya zaten bekleyen bir davet var! Lütfen başka birini seçin veya daveti geri çekin.',
          severity: 'error',
        });
      } else {
        setSnackbar({
          open: true,
          message: 'Davet gönderilemedi: ' + (errorMsg || 'Bir hata oluştu'),
          severity: 'error',
        });
      }
    }
    ...
    try {
      ...
      if (
        ...
        setSnackbar({
          open: true,
          message:
            'Davet geri çekme özelliği şu anda desteklenmiyor veya backend\'de bu endpoint yok. Lütfen yöneticinize başvurun.',
          severity: 'error',
        });
      } else {
        setSnackbar({
          open: true,
          message: 'Davet geri çekilemedi: ' + (errorMsg || 'Bir hata oluştu'),
          severity: 'error',
        });
      }
    }
      ...
      <div className="min-h-screen bg-gradient-to-br from-[#fff6f6] via-[#f4f6f8] to-[#fff6f6] p-6 md:p-12 font-sans text-black">
        <div className="max-w-6xl mx-auto space-y-10">
          <header className="flex flex-col items-center text-center space-y-4 pt-4 pb-8">
            ...
            <div>
              <h1 className="text-4xl font-extrabold text-[#a82936] tracking-tight">Yapay Zeka ile Ekip Arkadaşı Bul</h1>
                          ...
                          <button
                            ...
                            className={`w-full text-left p-4 rounded-xl border transition-all ${
                              selectedProject === proj._id
                                ? 'bg-[#f4e6e8] border-[#a82936] ring-1 ring-[#a82936]'
                                : 'bg-white/50 border-slate-200 hover:border-[#a82936]/40'
                            }`}
                          >
                            <p
                              className={`text-sm font-semibold ${
                                selectedProject === proj._id ? 'text-[#a82936]' : 'text-black'
                              }`}
                            >
                              {proj.title}
                            </p>
            ...
            <div className="lg:col-span-7 bg-white/90 backdrop-blur-xl p-8 rounded-3xl shadow-[0_8px_30px_rgba(107,15,26,0.06)] border border-[#6b0f1a]/10 min-h-[500px] flex flex-col relative overflow-hidden">
              {!result && !isAnalyzing && (
                <div className="h-full flex flex-col items-center justify-center text-slate-400 space-y-6 m-auto">
                  ...
                  <p className="text-center px-8 text-sm font-medium leading-relaxed">
                    Proje seçimi yaptıktan sonra yapay zeka veritabanındaki<br />
                    öğrenc i profillerini tarayarak en uygun adayları listeleyecektir.
                  </p>
                </div>
              )}
              ...
              {result && !isAnalyzing && (
                <div className="space-y-6 animate-in fade-in slide-in-from-bottom-8 duration-700">
                  {inputType === 'custom' && result?.extractedSkills?.length > 0 && (
                  ...
                  <div className="bg-[#f4e6e8]/60 border border-[#a82936]/20 p-5 rounded-2xl">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {result.extractedSkills.map((skill, idx) => (
                        <span
                          key={idx}
                          className="px-3 py-1 bg-white text-black text-xs font-bold rounded-full border border-[#a82936]/30 shadow-sm"
                        >
                          {skill}
                        </span>
                      ))}
                  ...
                  <div className="space-y-4">
                    ...
                    {result.candidates.map((candidate) => (
                      <div
                        key={candidate.id}
                        className="bg-white border border-[#a82936]/20 p-5 rounded-2xl shadow-sm hover:shadow-[#a82936]/10 hover:shadow-md transition-shadow"
                      >
                          ...
                          <div>
                            ...
                            <div className="flex flex-wrap gap-1.5">
                              {candidate.matchedSkills.map((s, i) => (
                                <span
                                  key={i}
                                  className="px-2.5 py-1 bg-[#fff6f6] text-black text-[10px] font-bold rounded-md border border-[#a82936]/30"
                                >
                                  {s}
                                </span>
                              ))}
                            ...
                            <div>
                              ...
                              <div className="flex flex-wrap gap-1.5">
                                {candidate.missingSkills.map((s, i) => (
                                  <span
                                    key={i}
                                    className="px-2.5 py-1 bg-[#f4e6e8] text-black/80 text-[10px] font-bold rounded-md border border-[#a82936]/20"
                                  >
                                    {s}
                                  </span>
                                ))}
